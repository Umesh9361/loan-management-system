...
- **QR Code Security**: Dual QR format system — (1) Estimate Receipt QR: `LMS:CODE:XXXXXX` format with 6-digit DB-backed lifetime codes stored in `estimate_codes` table, always 18 chars regardless of single/multi-loan (server restart safe, tenant-isolated, duplicate-prevention); (2) Label QR: `LMS:` prefix + Base64 encoded reversed UUID for sticker labels (browser engine). Backward compatible with old UUID QR and URL-format QR labels. Utility in `client/src/lib/qr-utils.ts` (encodeQrData/decodeQrData/encodeCodeQr/extractCode).
- **Inventory Scanning**: QR-based inventory verification with camera scanner, device scanner auto-detection, manual account number entry, and professional A4 PDF/print reports.
