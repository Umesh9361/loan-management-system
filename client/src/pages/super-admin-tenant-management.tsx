...
export default function SuperAdminTenantManagement() {
  ...
  const deleteTenantMutation = useMutation({
    mutationFn: async (tenantId: string) => {
      const response = await apiRequest("DELETE", `/api/super-admin/delete-tenant/${tenantId}`);
      return response;
    },
  ...
  const resetAdminPasswordMutation = useMutation({
    mutationFn: async ({ adminId, newPassword }: { adminId: string; newPassword: string }) => {
      await apiRequest("POST", `/api/super-admin/reset-admin-password/${adminId}`, { newPassword });
    },
    onSuccess: () => {
      toast({
  ...
  const deleteAdminMutation = useMutation({
    mutationFn: async (adminId: string) => {
      await apiRequest("DELETE", `/api/super-admin/delete-admin/${adminId}`);
    },
    onSuccess: () => {
      toast({
  ...
  const approvePasswordResetMutation = useMutation({
    mutationFn: async ({ requestId, newPassword }: { requestId: string; newPassword: string }) => {
      await apiRequest("POST", `/api/super-admin/approve-password-reset/${requestId}`, { newPassword });
    },
    onSuccess: () => {
      toast({
