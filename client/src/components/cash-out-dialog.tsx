...
import { ACCOUNT_TYPES, getCashOutAccount } from "@shared/constants";
...
export default function CashOutDialog({ open, onOpenChange }: CashOutDialogProps) {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      const description = useDualEntry 
        ? `रोकड दिलेली नोंद झाली आहे - द्विनोंदणी जर्नल ${response.journalEntry?.journalNumber || 'तयार'} झाले`
        : "रोकड दिलेली साधी नोंद झाली आहे";
      toast({
        title: "यशस्वी!",
  ...
  const onSubmit = (data: CashOutFormData) => {
    const toAccount = selectedParty?.name || getCashOutAccount(!!selectedParty, data.category);
    ...
    // Only add dual-entry fields if enabled
    if (useDualEntry) {
      Object.assign(requestData, {
        fromAccount: ACCOUNT_TYPES.CASH, // Money goes FROM our Cash account  
        toAccount: toAccount, // Professional account determination
      });
    }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px] max-h-[90vh] overflow-y-auto" aria-describedby="cash-out-description">
        <DialogHeader>
          <DialogTitle className="text-red-700">पैसे दिले</DialogTitle>
          <DialogDescription>
            रोकड दिलेली नोंद करा
          </DialogDescription>
        </DialogHeader>
        <div id="cash-out-description" className="sr-only">
          Cash out transaction entry form
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" autoComplete="off">
          ...
          <div>
            ...
            <Textarea
              id="narration"
              placeholder="तपशील लिहा (उदा: घरगुती खर्च, व्यापारी पेमेंट)"
              tabIndex={4}
              {...register("narration")}
            />
          </div>
