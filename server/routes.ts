...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.put("/api/company/interest-rate-warning-toggle", requireAuth, async (req: any, res) => {
    try {
      if (req.session.role !== 'admin' && req.session.role !== 'super_admin') {
        return res.status(403).json({ message: "फक्त अ‍ॅडमिन हे बदलू शकतो" });
      }
      ...
      const tenantId = req.session.tenantId!;
      const company = await storage.updateCompany(tenantId, { interestRateWarningEnabled: enabled });
      if (!company) {
        return res.status(404).json({ message: "कंपनी सापडली नाही" });
      }
      invalidateCache('company', tenantId);
      ...
      console.error("Interest rate warning toggle error:", error);
      res.status(500).json({ message: "सेटिंग बदलताना त्रुटी झाली" });
    }
  });

  app.get("/api/company/interest-settings", requireAuth, async (req: any, res) => {
    try {
      const tenantId = req.session.tenantId!;
      const company = await storage.getCompany(tenantId);
      if (!company) {
      ...
      console.error("Get interest settings error:", error);
      res.status(500).json({ message: "सेटिंग्स मिळवताना त्रुटी झाली" });
    }
  });

  app.put("/api/company/interest-settings", requireAuth, async (req: any, res) => {
    try {
      if (req.session.role !== 'admin' && req.session.role !== 'super_admin') {
        return res.status(403).json({ message: "फक्त अ‍ॅडमिन हे बदलू शकतो" });
      }
      ...
      if (interestType === 'advanced_compound') {
        if (!validFreqs.includes(compoundingFrequency)) {
          return res.status(400).json({ message: "Invalid compounding frequency" });
        }
        if (!validModes.includes(advancedCalculationMode)) {
  ...
  app.post("/api/loans", requireAuth, cacheBuster(['dashboard:', 'loans:', 'borrowers:']), async (req, res) => {
    try {
      ...
      const dateWarningConfirmed = req.body.dateWarningConfirmed === true;
      ...
      // 🔔 PRE-SAVE DATE CHECK: If not confirmed, check date and return warning if needed
      if (!dateWarningConfirmed) {
        try {
          const tenantId = req.session.tenantId!;
          ...
          if (loanDateObj > todayUTC) {
            ...
            warningMessage = `कर्ज वितरण तारीख ${dd}/${mm}/${yyyy} ही आजच्या पुढची (भविष्यातील) आहे! कृपया तारीख तपासा.`;
          } else {
            ...
            if (!companyData?.dataEntryMode && loanData.groupId) {
              ...
              if (lastGroupLoan.length > 0) {
                ...
                if (diffDays > 5) {
                  ...
                  warningMessage = `कर्ज वितरण तारीख ${dd1}/${mm1}/${yyyy1} ही ${groupName} ग्रुपच्या शेवटच्या कर्ज तारखेपासून (${dd2}/${mm2}/${yyyy2}) ${diffDays} दिवसांचा फरक आहे. कृपया तारीख तपासा.`;
                }
              }
            }
          }
          ...
          console.error('Date pre-check error (proceeding with save):', checkErr);
        }
      }
      ...
      // 🔔 POST-SAVE: If user confirmed date warning, log it in activity log + notification bell
      if (dateWarningConfirmed) {
        try {
          const tenantId = req.session.tenantId!;
          const loanDateObj = new Date(loanData.loanDate + 'T00:00:00Z');
          const dd = String(loanDateObj.getUTCDate()).padStart(2, '0');
          const mm = String(loanDateObj.getUTCMonth() + 1).padStart(2, '0');
          const yyyy = loanDateObj.getUTCFullYear();
          const warningMsg = `तारीख चेतावणी कन्फर्म: कर्ज वितरण तारीख ${dd}/${mm}/${yyyy} - खाते क्र. ${loan.accountNumber} - ${loan.borrowerName} - ₹${loan.principalAmount}. यूजरने कन्फर्म करून सेव केले.`;
          await storage.logUserActivity({
            userId: req.session.userId!, tenantId, activityType: 'date_warning',
            description: warningMsg,
            metadata: JSON.stringify({ warningType: 'loan_disbursement_date', severity: 'confirmed', loanId: loan.id, accountNumber: loan.accountNumber, borrowerName: loan.borrowerName, loanDate: loanData.loanDate, groupId: loanData.groupId })
          });
          await db.insert(notificationWarnings).values({
            tenantId, warningType: 'loan_disbursement_date', severity: 'warning',
            title: `⚠️ तारीख चेतावणी कन्फर्म - कर्ज वितरण`,
            message: warningMsg,
            metadata: JSON.stringify({ loanId: loan.id, accountNumber: loan.accountNumber, borrowerName: loan.borrowerName, loanDate: loanData.loanDate })
          });
          console.log(`🔔 DATE WARNING CONFIRMED: Loan ${loan.accountNumber}`);
        } catch (e) { console.error('Date warning log error:', e); }
      }
  ...
  app.post("/api/loans/:id/close", requireAuth, cacheBuster(['dashboard:', 'loans:']), async (req, res) => {
    try {
      ...
      delete requestData.dateWarningConfirmed;
      ...
      // 🔔 PRE-SAVE DATE CHECK for closure
      if (!dateWarningConfirmed) {
        try {
          const tenantId = req.session.tenantId!;
          ...
          if (closureDateObj > todayUTC) {
            ...
            warningTitle = `⚠️ भविष्यातील तारीख - कर्ज बंद`;
            warningMessage = `कर्ज बंद तारीख ${dd}/${mm}/${yyyy} ही आजच्या पुढची (भविष्यातील) आहे! कृपया तारीख तपासा.`;
          } else {
            const [companyData] = await db.select({ dataEntryMode: companies.dataEntryMode })
              .from(companies)
              .where(eq(companies.tenantId, tenantId));
            ...
            if (!companyData?.dataEntryMode) {
              ...
              if (lastClosure.length > 0) {
                ...
                if (diffDays > 5) {
                  ...
                  warningTitle = `⚠️ तारीख फरक - कर्ज बंद`;
                  warningMessage = `कर्ज बंद तारीख ${dd1}/${mm1}/${yyyy1} ही शेवटच्या कर्ज बंद तारखेपासून (${dd2}/${mm2}/${yyyy2}) ${diffDays} दिवसांचा फरक आहे. कृपया तारीख तपासा.`;
                }
              }
            }
          }
          ...
          console.error('Closure date pre-check error (proceeding with save):', checkErr);
        }
      }
      ...
      // 🔔 POST-SAVE: If user confirmed date warning for closure, log it
      if (dateWarningConfirmed) {
        try {
          const tenantId = req.session.tenantId!;
          const closureDateObj = new Date(closureData.closureDate + 'T00:00:00Z');
          const dd = String(closureDateObj.getUTCDate()).padStart(2, '0');
          const mm = String(closureDateObj.getUTCMonth() + 1).padStart(2, '0');
          const yyyy = closureDateObj.getUTCFullYear();
          const warningMsg = `तारीख चेतावणी कन्फर्म: कर्ज बंद तारीख ${dd}/${mm}/${yyyy} - खाते क्र. ${loanDetails.accountNumber} - ${loanDetails.borrowerName} - ₹${closureData.totalAmount}. यूजरने कन्फर्म करून सेव केले.`;
          await storage.logUserActivity({
            userId: req.session.userId!, tenantId, activityType: 'date_warning',
            description: warningMsg,
            metadata: JSON.stringify({ warningType: 'loan_closure_date', severity: 'confirmed', loanId: id, accountNumber: loanDetails.accountNumber, borrowerName: loanDetails.borrowerName, closureDate: closureData.closureDate })
          });
          await db.insert(notificationWarnings).values({
            tenantId, warningType: 'loan_closure_date', severity: 'warning',
            title: `⚠️ तारीख चेतावणी कन्फर्म - कर्ज बंद`,
            message: warningMsg,
            metadata: JSON.stringify({ loanId: id, accountNumber: loanDetails.accountNumber, borrowerName: loanDetails.borrowerName, closureDate: closureData.closureDate })
          });
          console.log(`🔔 DATE WARNING CONFIRMED: Closure ${loanDetails.accountNumber}`);
        } catch (e) { console.error('Closure date warning log error:', e); }
      }
  ...
  app.get('/api/qr-generate', requireAuth, async (req: any, res) => {
    try {
      ...
      const qrContent = url || data;
      if (!qrContent) return res.status(400).json({ error: 'url or data required' });
      const dataUrl = await QRCode.toDataURL(String(qrContent), {
        width: Math.min(1024, Math.max(64, parseInt(String(size)) || 256)),
        margin: 2,
